...
export function MobileLayoutProvider({
  ...
  const [selectedChatId, setSelectedChatIdState] = useState<string | null>(initialChatId)
...
export function MobileLayout({ className, children, ...props }: MobileLayoutProps) {
  return (
    <div
      className={cn(
  ...
  return (
    <div
      className={cn(
        ...
        // Slide animations
        view === 'chat-list' && (isVisible ? 'translate-x-0' : '-translate-x-full'),
        view === 'message-thread' && (isVisible ? 'translate-x-0' : 'translate-x-full'),
        // Z-index layering
        view === 'chat-list' && 'z-10',
  ...
  return (
    <>
      ...
      >
        ...
        {/* Panel Content */}
        <div className="flex-1 overflow-y-auto safe-area-bottom">{children}</div>
      </aside>
    </>
  )
}
...
export function MobileActionButton({ className, icon, label, ...props }: MobileActionButtonProps) {
  return (
    <button
      className={cn(
