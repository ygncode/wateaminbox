 ...
 *  2. Permission: `can_send_messages` is a hard prerequisite for every
 *     other state - without it, Open/Reopen/Take over would all just 403
 *     server-side, so none of them are offered.
 *  3. Assignment: a contact actively assigned to someone else blocks
 ...
 *  4. Lifecycle: a resolved conversation (or one that's never been
