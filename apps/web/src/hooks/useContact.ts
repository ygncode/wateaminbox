...
export function useUpdateContact() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      const response = await api.patch<ContactDetail>(`/contacts/${contactId}`, {
        customName,
        notesShared,
      })
      return response
...
export function useUpdatePrivateNotes() {
  ...
  return useMutation({
    mutationFn: async ({ contactId, content }: { contactId: string; content: string }) => {
      const response = await api.post<PrivateNote>(`/contacts/${contactId}/notes/private`, {
        content,
      })
      return response
    },
...
export function useCreateSharedNote() {
  ...
  return useMutation({
    mutationFn: async ({ contactId, content }: { contactId: string; content: string }) => {
      const response = await api.post<SharedNote>(`/contacts/${contactId}/notes/shared`, {
        content,
      })
      return response
    },
...
export function useUpdateSharedNote() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      const response = await api.put<SharedNote>(`/contacts/${contactId}/notes/shared/${noteId}`, {
        content,
      })
      return response
    },
