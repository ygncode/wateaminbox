...
export function useSwipeGesture<T extends HTMLElement>(
  ...
  const handleTouchMove = useCallback(
    (event: TouchEvent) => {
      if (!enabled || !isTouchActiveRef.current || !touchStartRef.current) return
...
export function useSwipeGestureCallback(options: SwipeGestureOptions = {}) {
  ...
  const handlers = {
    ...
    onTouchEnd: useCallback(
      ...
      [enabled, threshold, timeout, onSwipe, onSwipeLeft, onSwipeRight, onSwipeUp, onSwipeDown]
    ),
  }
...
export function useSwipeProgress<T extends HTMLElement>(
  ...
  }, [ref, enabled, direction, maxDistance, threshold, onProgressChange, onSwipeComplete])
}
