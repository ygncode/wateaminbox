  ...
  return useMutation({
    ...
    onMutate: async ({ messageId, conversationId }) => {
      ...
      queryClient.setQueryData<Message[]>(
        ...
        (old) => old?.map((msg) => (msg.id === messageId ? { ...msg, isDeleted: true } : msg)) || []
      )
...
export function useStarMessage() {
  ...
  return useMutation({
    ...
    onMutate: async ({ messageId, conversationId, isStarred }) => {
      ...
      queryClient.setQueryData<Message[]>(
        messageKeys.list(conversationId),
        (old) => old?.map((msg) => (msg.id === messageId ? { ...msg, isStarred } : msg)) || []
      )
    ...
    onError: (_err, _variables, context) => {
      if (context?.previousMessages) {
        queryClient.setQueryData(messageKeys.list(context.conversationId), context.previousMessages)
      }
    },
  })
}
