...
import { connectWhatsApp, disconnectWhatsApp, getWhatsAppStatus } from '../lib/api'
import type {
  QRCodePayload,
...
export function useWhatsAppConnection(): WhatsAppConnection {
  ...
  const waitingQrTimeoutRef = useRef<ReturnType<typeof setTimeout> | null>(null)
  ...
  useEffect(() => {
    if (status && !isConnectingRef.current) {
      if (status.status === 'connected') {
        ...
        if (wsConnected && !connectMutation.isPending && !hasTriggeredAutoConnectRef.current) {
