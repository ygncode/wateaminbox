...
export function useUpdateGroup() {
  ...
  return useMutation({
    mutationFn: async ({ groupId, customName }: { groupId: string; customName?: string }) => {
...
export function useGroupAdminStatus(groupId: string | null) {
  return useQuery({
    ...
    queryFn: async () => {
      if (!groupId) throw new Error('No group ID provided')
      const response = await api.get<GroupAdminStatus>(`/groups/${groupId}/admin-status`)
      return response
    },
    enabled: !!groupId,
    staleTime: 30_000, // 30 seconds
    ...
    onSuccess: (_data, variables) => {
      ...
      queryClient.setQueryData(
        ...
        (old: GroupDetail | undefined) => {
          ...
          return {
            ...old,
            participants: old.participants.filter((p) => p.jid !== variables.participantJid),
