...
function getHeaders(): Record<string, string> {
  ...
  if (!companyId) {
    throw new Error('No company selected. Please select a company to continue.')
  }
...
export function useExportContacts() {
  return useMutation({
    mutationFn: async ({
      ...
      const response = await fetch(`${API_BASE_URL}/export/contacts?${params}`, {
        headers: getHeaders(),
        credentials: 'include',
      })
...
export function useExportMessages() {
  return useMutation({
    mutationFn: async ({
      ...
      if (filters.messageTypes?.length) params.set('messageTypes', filters.messageTypes.join(','))
      if (filters.limit) params.set('limit', String(filters.limit))

      const response = await fetch(`${API_BASE_URL}/export/messages?${params}`, {
        headers: getHeaders(),
        credentials: 'include',
      })
...
export function useExportConversation() {
  return useMutation({
    mutationFn: async ({
      ...
      const response = await fetch(`${API_BASE_URL}/export/conversation/${contactId}?${params}`, {
        headers: getHeaders(),
        credentials: 'include',
      })
