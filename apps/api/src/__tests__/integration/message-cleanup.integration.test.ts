...
const companyMessageData = new Map<string, ReturnType<typeof createMockMessage>[]>()
    ...
    updateTable: mock((table: string) => {
      if (table !== 'messages') {
        return {
          set: mock(() => ({
            where: mock(() => ({
              execute: mock(() => Promise.resolve({ numUpdatedRows: BigInt(0) })),
            })),
          })),
        }
      }
...
const mockTenantSchemaExistsFunc = mock(async (companyId: string): Promise<boolean> => {
 ...
 */
async function runCleanupForSingleCompany(companyId: string, messages: ReturnType<typeof createMockMessage>[]) {
...
describe('MessageCleanupService - Integration Tests', () => {
  ...
  describe('Multi-Tenant Cleanup', () => {
    it('should process cleanup across multiple tenant schemas', async () => {
      ...
      const company1Result = result.companies.find((c) => c.companyId === 'company-1')
      const company2Result = result.companies.find((c) => c.companyId === 'company-2')
      const company3Result = result.companies.find((c) => c.companyId === 'company-3')
    ...
    it('should handle tenants with missing schemas gracefully', async () => {
      ...
      const noSchemaResult = result.companies.find((c) => c.companyId === 'company-no-schema')
    ...
    it('should handle empty tenant schemas without errors', async () => {
      ...
      const emptyTenantResult = result.companies.find((c) => c.companyId === 'empty-tenant')
    ...
    it('should continue processing other tenants if one has errors', async () => {
      ...
      const ok1Result = result.companies.find((c) => c.companyId === 'company-ok-1')
      expect(ok1Result?.expiredCount).toBe(1)
      expect(ok1Result?.error).toBeUndefined()

      // company-ok-2 had no messages
      const ok2Result = result.companies.find((c) => c.companyId === 'company-ok-2')
  ...
  describe('Edge Case Handling', () => {
    it('should handle tenant with very old pending messages', async () => {
      ...
      const result = await runCleanupForSingleCompany('company-old-messages', veryOldMessages)

      expect(result.totalProcessed).toBe(1)
      expect(result.totalExpired).toBe(1)

      const companyResult = result.companies.find((c) => c.companyId === 'company-old-messages')
    ...
    it('should skip messages with existing metadata', async () => {
      ...
      const result = await runCleanupForSingleCompany('company-with-metadata', messagesWithMetadata)

      // Should not expire messages with existing metadata
      // The query filters for metadata IS NULL
      expect(result.totalExpired).toBe(0)

      const companyResult = result.companies.find((c) => c.companyId === 'company-with-metadata')
    ...
    it('should only target from_me=true messages', async () => {
      ...
      const result = await runCleanupForSingleCompany('company-mixed', mixedMessages)

      // Should only expire from_me=true messages
      expect(result.totalExpired).toBe(1)

      const companyResult = result.companies.find((c) => c.companyId === 'company-mixed')
      expect(companyResult?.expiredCount).toBe(1)
    })
    ...
    it('should handle non-pending messages correctly', async () => {
      ...
      const result = await runCleanupForSingleCompany('company-non-pending', nonPendingMessages)

      // Should not expire non-pending messages
      expect(result.totalExpired).toBe(0)

      const companyResult = result.companies.find((c) => c.companyId === 'company-non-pending')
      expect(companyResult?.expiredCount).toBe(0)
    })
    ...
    it('should handle large numbers of messages', async () => {
      ...
      const result = await runCleanupForSingleCompany('company-many-messages', manyStaleMessages)

      // Should process all stale messages
      expect(result.totalExpired).toBe(150)

      const companyResult = result.companies.find((c) => c.companyId === 'company-many-messages')
  ...
  describe('Manual Trigger', () => {
    it('should trigger cleanup for a specific company via cleanupCompanyMessages', async () => {
      ...
      const expiredCount = await cleanupCompanyMessages('company-manual', 5, 100)
    ...
    it('should handle manual trigger for company with no stale messages', async () => {
      ...
      // Manually trigger cleanup for specific company
      const expiredCount = await cleanupCompanyMessages('company-no-stale', 5, 100)
  ...
  describe('WebSocket Broadcasts', () => {
    it('should broadcast status updates for expired messages', async () => {
      ...
      const result = await runCleanupForSingleCompany('company-broadcast', staleMessages)
    ...
    it('should group broadcasts by contact_id', async () => {
      ...
      const result = await runCleanupForSingleCompany('company-multiple-contacts', messagesForMultipleContacts)
