    ...
    updateTable: mock((table: string) => {
      if (table === "whatsapp_catalogs") {
        ...
        builder.execute = mock(() => {
          ...
          return Promise.resolve({
            numUpdatedRows: BigInt(idx !== -1 ? 1 : 0),
          });
        });
    ...
    app.get("/catalogs/status", async (c) => {
      ...
      return c.json({
        totalCatalogs: Number(
          (catalogsResult as Record<string, unknown>)?.count ?? 0,
        ),
        activeCatalogs: Number(
          (activeCatalogsResult as Record<string, unknown>)?.count ?? 0,
        ),
        totalProducts: Number(
          (productsResult as Record<string, unknown>)?.count ?? 0,
        ),
        lastSyncAt:
          (lastSyncResult as MockCatalog | undefined)?.last_synced_at ?? null,
      });
    });
  });
