...
export function createTestWebSocketClient(
  ...
  client.ws.onmessage = (event) => {
    try {
      ...
      console.log(
        `[Test Client] Event received: ${type} (count: ${currentCount + 1})`,
      );
    } catch (error) {
...
export async function runWebSocketTest(
  ...
  results.push({
    ...
    message:
      client.ws.readyState === WebSocket.OPEN ? "Connected" : "Not connected",
  });
  ...
  results.push({
    ...
    passed: authCount <= 1,
    message:
      authCount <= 1 ? "No duplicates" : `Duplicate auth events: ${authCount}`,
  });
