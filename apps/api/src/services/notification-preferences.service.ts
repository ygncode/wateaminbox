...
export async function muteContact(
  ...
  const updated = await tenantDb
    ...
    .set({
      muted_contacts: sql<
        string[]
      >`array_append(COALESCE(muted_contacts, ARRAY[]::TEXT[]), ${normalizedJid})`,
      updated_at: new Date(),
    })
    .where("user_id", "=", userId)
    .where(
      sql<SqlBool>`NOT (COALESCE(muted_contacts, ARRAY[]::TEXT[]) @> ARRAY[${normalizedJid}]::text[])`,
    )
    .returningAll()
    .executeTakeFirst();
...
export async function unmuteContact(
  ...
  const updated = await tenantDb
    ...
    .set({
      muted_contacts: sql<
        string[]
      >`array_remove(COALESCE(muted_contacts, ARRAY[]::TEXT[]), ${normalizedJid})`,
      updated_at: new Date(),
    })
    .where("user_id", "=", userId)
    .where(
      sql<SqlBool>`COALESCE(muted_contacts, ARRAY[]::TEXT[]) @> ARRAY[${normalizedJid}]::text[]`,
    )
    .returningAll()
    .executeTakeFirst();
