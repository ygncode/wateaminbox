...
import { getCleanupConfig, type MessageCleanupConfig } from '../config/cleanup.config.js'
...
export async function initializeMessageCleanup(
  ...
  cleanupIntervalId = setInterval(async () => {
    try {
      const result = await runCleanupCycle()
      if (result.skipped) {
        console.log(`[MessageCleanup] Cleanup cycle skipped (no active companies)`)
      } else {
        console.log(
...
export async function runCleanupCycle(): Promise<CleanupCycleResult> {
  ...
  for (const company of companies) {
    try {
      ...
      console.error(`[MessageCleanup] Failed to cleanup company ${company.id}:`, error)
    }
  }
...
export async function cleanupCompanyMessages(
  ...
  if (!schemaExists) {
    console.warn(`[MessageCleanup] Tenant schema does not exist for company ${companyId}`)
    return 0
  }
...
export async function triggerCleanupForCompany(
  ...
  return await cleanupCompanyMessages(companyId, timeout, currentConfig.batchSize)
}
