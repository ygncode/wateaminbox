...
legacyRoutes.post(
  ...
  async (c) => {
    ...
    try {
      const result = await whatsappService.spawnConnection(tenantDb, companyId, user.id)
 ...
 */
legacyRoutes.get('/status', authMiddleware, tenantFromHeader('X-Company-ID'), async (c) => {
  const tenantDb = c.get('tenantDb')
...
legacyRoutes.post(
  ...
  async (c) => {
    ...
    try {
      const result = await whatsappService.sendMessage(tenantDb, companyId, user.id, {
        jid: input.jid,
 ...
 */
legacyRoutes.get('/connection', authMiddleware, tenantFromHeader('X-Company-ID'), async (c) => {
  const tenantDb = c.get('tenantDb')
