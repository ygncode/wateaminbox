 ...
 */
statusRoutes.get('/limits', authMiddleware, tenantFromHeader('X-Company-ID'), async (c) => {
 ...
 */
statusRoutes.get('/sync-status', authMiddleware, tenantFromHeader('X-Company-ID'), async (c) => {
  const tenantDb = c.get('tenantDb')
  ...
  try {
    ...
    if (staleConnectionIds.length > 0) {
      ...
      tenantDb
        ...
        .execute()
        .catch((err) => {
          logger.error({ err: formatError(err) }, 'Failed to auto-reset stale sync status')
        })
    }
 ...
 */
statusRoutes.post('/sync-reset', authMiddleware, tenantFromHeader('X-Company-ID'), async (c) => {
  const tenantDb = c.get('tenantDb')
