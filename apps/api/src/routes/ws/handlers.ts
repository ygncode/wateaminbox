 ...
 */
async function handleSendMessage(ws: WebSocketConnection, payload: unknown): Promise<void> {
...
export async function handleClientMessage(ws: WebSocketConnection, message: string): Promise<void> {
  ...
  switch (parsed.type) {
    case 'auth': {
      ...
      await authenticateConnection(ws, parsed.payload.token, parsed.payload.companyId)
      break
    }
