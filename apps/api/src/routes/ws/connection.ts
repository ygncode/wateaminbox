...
export function getCompanyConnections(
  companyId: string,
): Set<WebSocketConnection> | undefined {
...
export function broadcastToCompany(
  ...
  if (companyConnections) {
      ...
      logger.debug(
        {
          sentCount,
          companyId,
          connectionId: (message as { connectionId?: string }).connectionId,
        },
    ...
    if (message.type === "qr") {
      logger.warn(
        { companyId },
        "No active WebSocket connections for company to broadcast QR code",
      );
    }
  }
}
