...
mediaRoutes.post('/download/:messageId', async (c) => {
  ...
  try {
    ...
    logger.info({ messageId, companyId, connectionId: connection.id }, 'Published download request')
...
mediaRoutes.post('/upload', uploadRateLimiter, async (c) => {
  ...
  try {
    ...
    const uploadResult = await uploadMedia(buffer, file.type, companyId, file.name)
