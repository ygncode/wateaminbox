 ...
 */
customerChatsRoutes.get("/:id/chats", requireContactVisibility(), async (c) => {
  const { tenantDb, permissions, companyId } = getRouteContext(c);
  ...
  // Signed here rather than in the service: a stored media reference is not
  // fetchable by a browser, and the workspace the signature is scoped to is
  // the route's to know.
  const chatsWithAvatars = await Promise.all(
    visible
      ...
      .map(async (chat) => ({
        ...chat,
        avatarUrl: await getAuthorizedMediaUrlOrNull(chat.avatarUrl, companyId),
      })),
  );
  return successData(c, { chats: chatsWithAvatars });
});
