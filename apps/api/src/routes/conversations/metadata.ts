...
metadataRoutes.get("/:id/notes", async (c) => {
  ...
  if (
    ...
    return notFound(c, "Conversation notes");
  }
  const notes = await tenantDb
    .selectFrom("conversation_notes")
    .selectAll()
    .where("conversation_id", "=", c.req.param("id"))
    .where((eb) =>
      eb.or([
...
metadataRoutes.get("/:id/tags", async (c) => {
  ...
  if (
    ...
    return notFound(c, "Conversation tags");
  }
  const tags = await tenantDb
    ...
    .where("link.conversation_id", "=", c.req.param("id"))
    .orderBy("tag.name")
    .execute();
...
metadataRoutes.post("/:id/tags", zValidator("json", tagSchema), async (c) => {
  ...
  await tenantDb
    ...
    .values({ conversation_id: c.req.param("id"), tag_id: tagId })
