...
- `MEDIA_DOMAIN`: authenticated or presigned S3 requests to MinIO. The MinIO
  console is disabled and is never routed.

PostgreSQL, NATS, Meilisearch, MinIO, Centrifugo, and the orchestrator have no
...
The media bucket is private. Browser access uses time-limited signatures through
`MEDIA_DOMAIN`; anonymous bucket access is explicitly disabled. Treat that
hostname as an object API, not a public file directory.
...
for name in postgres_password nats_token meilisearch_master_key \
  ...
  jwt_secret centrifugo_api_key centrifugo_token_secret; do
...
random token, Meilisearch requires a master key, and MinIO creates a distinct bucket-
scoped application user. Development credentials in `.env.example` and
...
$COMPOSE up -d postgres nats meilisearch minio minio-init centrifugo
...
2. **MinIO**: all media objects. Mirror to encrypted off-host object storage with
   versioning/object lock as policy requires. Preserve keys and checksums.
...
3. Restore media before resuming workers; retain a private bucket and the same
   object keys.
...
- MinIO health, PostgreSQL availability/replication, Meilisearch health;
