...
import { addColumnToAllTenants, dropColumnFromAllTenants } from './migration-helpers.js'

/**
 * Migration 020: Add title column to quick_replies table
 *
 * PROBLEM:
 * Migration 005 originally defined quick_replies with a 'title' column.
 * Migration 015 (the baseline fix) accidentally omitted the 'title' column
 * when rewriting setup_tenant_schema, causing a schema mismatch.
 *
 * SOLUTION:
 * 1. Add 'title' column to existing tenant schemas
 * 2. Update setup_tenant_schema function to include the 'title' column
 */
export async function up(db: Kysely<unknown>): Promise<void> {
  console.log('Adding title column to quick_replies table in all tenant schemas...')

  // Add title column to all existing tenant schemas
  ...
  // Update the setup_tenant_schema function to include title column for new tenants
  await sql`
  ...
  console.log('Updated setup_tenant_schema function with title column')
}

export async function down(db: Kysely<unknown>): Promise<void> {
  // Drop the title column from all tenant schemas
  await dropColumnFromAllTenants(db, 'quick_replies', 'title')

  console.log('Removed title column from quick_replies table')
}
