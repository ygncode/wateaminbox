...
/**
 * This migration adds WhatsApp Business Catalogs support.
 * - Creates whatsapp_catalogs table to store product catalogs
 * - Creates catalog_products table to store products within catalogs
 * - Updates tenant schema function
 */
export async function up(db: Kysely<unknown>): Promise<void> {
  // Create catalog_status enum if not exists
  await sql`
    DO $$ BEGIN
  ...
  // Create product_visibility enum if not exists
  await sql`
    DO $$ BEGIN
      CREATE TYPE product_visibility AS ENUM ('visible', 'hidden');
    EXCEPTION
      WHEN duplicate_object THEN null;
    END $$;
  `.execute(db)

  // Update the setup_tenant_schema function to include catalog tables
  await sql`
    ...
    BEGIN
      ...
      -- Tags table (with WhatsApp label sync fields)
      EXECUTE format('
      ...
      -- WhatsApp labels table (for storing fetched labels from WhatsApp Business)
      EXECUTE format('
      ...
      -- WhatsApp catalogs table (for storing product catalogs from WhatsApp Business)
      EXECUTE format('
      ...
      -- Catalog products table (for storing products within catalogs)
      EXECUTE format('
      ...
      EXECUTE format('CREATE INDEX IF NOT EXISTS %I ON %I.tags (whatsapp_label_id)', schema_name || '_tags_label_id_idx', schema_name);
      EXECUTE format('CREATE INDEX IF NOT EXISTS %I ON %I.whatsapp_labels (label_id)', schema_name || '_wa_labels_id_idx', schema_name);
      EXECUTE format('CREATE INDEX IF NOT EXISTS %I ON %I.whatsapp_labels (synced_tag_id)', schema_name || '_wa_labels_tag_idx', schema_name);
      EXECUTE format('CREATE INDEX IF NOT EXISTS %I ON %I.whatsapp_catalogs (catalog_id)', schema_name || '_wa_catalogs_id_idx', schema_name);
      EXECUTE format('CREATE INDEX IF NOT EXISTS %I ON %I.whatsapp_catalogs (status)', schema_name || '_wa_catalogs_status_idx', schema_name);
      EXECUTE format('CREATE INDEX IF NOT EXISTS %I ON %I.catalog_products (catalog_id)', schema_name || '_catalog_products_catalog_idx', schema_name);
      EXECUTE format('CREATE INDEX IF NOT EXISTS %I ON %I.catalog_products (product_id)', schema_name || '_catalog_products_product_idx', schema_name);
    END;
    $$ LANGUAGE plpgsql;
  `.execute(db)

  // Add tables to existing tenant schemas
  ...
  for (const { schema_name } of schemas.rows) {
    // Create whatsapp_catalogs table if not exists
    const catalogTableExists = await sql<{ exists: boolean }>`
      SELECT EXISTS (
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = ${schema_name}
        AND table_name = 'whatsapp_catalogs'
      ) as exists
    `.execute(db)

    if (!catalogTableExists.rows[0]?.exists) {
      await sql.raw(`
        CREATE TABLE IF NOT EXISTS "${schema_name}".whatsapp_catalogs (
          id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
          catalog_id VARCHAR(100) NOT NULL UNIQUE,
          name VARCHAR(255) NOT NULL,
          description TEXT,
      ...
      await sql.raw(`
        CREATE INDEX IF NOT EXISTS "${schema_name}_wa_catalogs_id_idx"
        ON "${schema_name}".whatsapp_catalogs (catalog_id)
      `).execute(db)

      await sql.raw(`
        CREATE INDEX IF NOT EXISTS "${schema_name}_wa_catalogs_status_idx"
        ON "${schema_name}".whatsapp_catalogs (status)
      `).execute(db)
    }

    // Create catalog_products table if not exists
    const productsTableExists = await sql<{ exists: boolean }>`
      SELECT EXISTS (
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = ${schema_name}
    ...
    if (!productsTableExists.rows[0]?.exists) {
      await sql.raw(`
        CREATE TABLE IF NOT EXISTS "${schema_name}".catalog_products (
          id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
          product_id VARCHAR(100) NOT NULL,
          catalog_id VARCHAR(100) NOT NULL,
          name VARCHAR(255) NOT NULL,
      ...
      await sql.raw(`
        CREATE INDEX IF NOT EXISTS "${schema_name}_catalog_products_catalog_idx"
        ON "${schema_name}".catalog_products (catalog_id)
      `).execute(db)

      await sql.raw(`
        CREATE INDEX IF NOT EXISTS "${schema_name}_catalog_products_product_idx"
        ON "${schema_name}".catalog_products (product_id)
      `).execute(db)
    }
  }
}
...
export async function down(db: Kysely<unknown>): Promise<void> {
  ...
  // Remove the catalog tables from each tenant schema
  for (const { schema_name } of schemas.rows) {
    await sql.raw(`
      DROP TABLE IF EXISTS "${schema_name}".catalog_products
    `).execute(db)

    await sql.raw(`
      DROP TABLE IF EXISTS "${schema_name}".whatsapp_catalogs
    `).execute(db)

    await sql.raw(`
      DROP INDEX IF EXISTS "${schema_name}_catalog_products_catalog_idx"
    `).execute(db)

    await sql.raw(`
      DROP INDEX IF EXISTS "${schema_name}_catalog_products_product_idx"
    `).execute(db)

    await sql.raw(`
      DROP INDEX IF EXISTS "${schema_name}_wa_catalogs_id_idx"
    `).execute(db)

    await sql.raw(`
      DROP INDEX IF EXISTS "${schema_name}_wa_catalogs_status_idx"
    `).execute(db)
  }

  // Drop enum types
  await sql`DROP TYPE IF EXISTS product_visibility`.execute(db)
  await sql`DROP TYPE IF EXISTS catalog_status`.execute(db)
}
