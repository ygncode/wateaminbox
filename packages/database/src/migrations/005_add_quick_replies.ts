...
/**
 * This migration adds quick replies for fast message responses.
 * - Creates quick_replies table in tenant schema
 * - Updates tenant schema function
 */
export async function up(db: Kysely<unknown>): Promise<void> {
  // Update the setup_tenant_schema function to include quick_replies table
  await sql`
  ...
  // Add quick_replies table to existing tenant schemas
  ...
  for (const { schema_name } of schemas.rows) {
    ...
    if (!tableExists.rows[0]?.exists) {
      await sql.raw(`
        CREATE TABLE IF NOT EXISTS "${schema_name}".quick_replies (
          id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
          shortcut VARCHAR(50) NOT NULL,
          title VARCHAR(255) NOT NULL,
          content TEXT NOT NULL,
      ...
      // Create indexes
      await sql.raw(`
        CREATE INDEX IF NOT EXISTS "${schema_name}_quick_replies_shortcut_idx"
        ON "${schema_name}".quick_replies (shortcut)
      `).execute(db)

      await sql.raw(`
        CREATE INDEX IF NOT EXISTS "${schema_name}_quick_replies_user_idx"
        ON "${schema_name}".quick_replies (created_by)
      `).execute(db)
    }
  }
}
...
export async function down(db: Kysely<unknown>): Promise<void> {
  ...
  // Drop the quick_replies table from each tenant schema
  for (const { schema_name } of schemas.rows) {
    await sql.raw(`
      DROP TABLE IF EXISTS "${schema_name}".quick_replies
    `).execute(db)

    await sql.raw(`
      DROP INDEX IF EXISTS "${schema_name}_quick_replies_shortcut_idx"
    `).execute(db)

    await sql.raw(`
      DROP INDEX IF EXISTS "${schema_name}_quick_replies_user_idx"
    `).execute(db)
  }
}
