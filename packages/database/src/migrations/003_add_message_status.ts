...
/**
 * This migration adds message status tracking for delivery receipts.
 * - Adds message_status enum type
 * - Adds status column to messages table
 * - Updates tenant schema function to include status column
 */
export async function up(db: Kysely<unknown>): Promise<void> {
  ...
  // Add status column to existing tenant schemas
  ...
  // Add the status column to each existing tenant schema's messages table
  for (const { schema_name } of schemas.rows) {
    ...
    if (!columnExists.rows[0]?.exists) {
      ...
      // Add index on message_id for faster receipt lookups
      await sql.raw(`
        CREATE INDEX IF NOT EXISTS "${schema_name}_messages_message_id_idx"
        ON "${schema_name}".messages (message_id)
      `).execute(db)
    }
  }
}
...
export async function down(db: Kysely<unknown>): Promise<void> {
  ...
  // Remove the status column from each tenant schema
  for (const { schema_name } of schemas.rows) {
    await sql.raw(`
      ALTER TABLE "${schema_name}".messages
    ...
    await sql.raw(`
      DROP INDEX IF EXISTS "${schema_name}_messages_message_id_idx"
    `).execute(db)
  }

  // Drop the message_status type
