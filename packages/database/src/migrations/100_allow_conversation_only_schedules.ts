...
/**
 * Allow scheduled messages to exist on a conversation without a contact row.
 */
export async function up(db: Kysely<unknown>): Promise<void> {
  await forEachTenant(db, async (schema) => {
    await sql`SET lock_timeout = '5s'`.execute(db);
    const table = sql.table(`${schema}.scheduled_messages`);
    await sql`ALTER TABLE ${table}
      ALTER COLUMN contact_id DROP NOT NULL`.execute(db);
    const constraint = "scheduled_messages_contact_or_conversation_check";
    const exists = await sql<{ exists: boolean }>`
      SELECT EXISTS (
        ...
        WHERE schema_record.nspname = ${schema}
          AND table_record.relname = 'scheduled_messages'
...
export async function down(db: Kysely<unknown>): Promise<void> {
  await forEachTenant(db, async (schema) => {
    const table = sql.table(`${schema}.scheduled_messages`);
    await sql`ALTER TABLE ${table}
      DROP CONSTRAINT IF EXISTS ${sql.ref("scheduled_messages_contact_or_conversation_check")}`.execute(
      db,
    );
    await sql`ALTER TABLE ${table}
