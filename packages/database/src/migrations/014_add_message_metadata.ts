...
/**
 * Migration 014: Add status and metadata columns to messages table
 *
 * This migration:
 * 1. Adds the 'status' column to messages in all existing tenant schemas (was missing from migration 013)
 * 2. Adds the 'metadata' column to messages for storing error details and other metadata
 * 3. Updates the setup_tenant_schema function to include these columns for new tenants
 *
 * The metadata column will store JSONB data including:
 * - error: error type (e.g., 'delivery_timeout')
...
export async function up(db: Kysely<unknown>): Promise<void> {
  ...
  for (const { schema_name } of schemas.rows) {
    // Add status column if missing (migration 013 accidentally removed it from the function)
    ...
    // Add metadata column for storing error details and other metadata
    ...
    if (!metadataColumnExists.rows[0]?.exists) {
      ...
      `.execute(db);
      console.log(`Added metadata column to ${schema_name}.messages`);
    }
  }

  // Update setup_tenant_schema function to include status and metadata columns
  await sql`
    ...
    BEGIN
      ...
      EXECUTE format('CREATE INDEX IF NOT EXISTS %I ON %I.messages (contact_id, timestamp DESC)', schema_name || '_messages_contact_timestamp_idx', schema_name);
      EXECUTE format('CREATE INDEX IF NOT EXISTS %I ON %I.contact_assignments (assigned_to) WHERE unassigned_at IS NULL', schema_name || '_contact_assignments_active_idx', schema_name);
      EXECUTE format('CREATE INDEX IF NOT EXISTS %I ON %I.messages (contact_id, from_me) WHERE from_me = false', schema_name || '_messages_incoming_idx', schema_name);
    END;
    $$ LANGUAGE plpgsql;
  `.execute(db);

  console.log("Updated setup_tenant_schema function with status and metadata columns");
}
...
export async function down(db: Kysely<unknown>): Promise<void> {
  ...
  for (const { schema_name } of schemas.rows) {
    // Drop metadata column (we want to keep status as it's used elsewhere)
    await sql`
      ALTER TABLE ${sql.raw(`"${schema_name}".messages`)}
  ...
  // Note: We keep the status column as it's used for delivery tracking
  // Revert setup_tenant_schema to version without metadata
  await sql`
    CREATE OR REPLACE FUNCTION setup_tenant_schema(schema_name TEXT)
    RETURNS void AS $$
    BEGIN
      ...
      -- Messages table (with status, without metadata)
      EXECUTE format('
        CREATE TABLE IF NOT EXISTS %I.messages (
          id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
          whatsapp_connection_id UUID,
  ...
  console.log("Reverted setup_tenant_schema function (removed metadata column)");
}
