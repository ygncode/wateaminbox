...
/**
 * This migration adds WhatsApp Business labels sync support.
 * - Adds whatsapp_label_id and synced_at to tags table
 * - Creates whatsapp_labels table to store fetched WhatsApp labels
 * - Updates tenant schema function
 */
export async function up(db: Kysely<unknown>): Promise<void> {
  // Update the setup_tenant_schema function to include label sync fields
  await sql`
  ...
  // Add columns to existing tenant schemas
  ...
  for (const { schema_name } of schemas.rows) {
    // Add whatsapp_label_id column to tags table if not exists
    const labelIdExists = await sql<{ exists: boolean }>`
      SELECT EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_schema = ${schema_name}
        AND table_name = 'tags'
        AND column_name = 'whatsapp_label_id'
      ) as exists
    `.execute(db)

    if (!labelIdExists.rows[0]?.exists) {
      await sql.raw(`
        ALTER TABLE "${schema_name}".tags
        ADD COLUMN whatsapp_label_id VARCHAR(100),
        ADD COLUMN synced_at TIMESTAMPTZ
      `).execute(db)

      await sql.raw(`
        CREATE INDEX IF NOT EXISTS "${schema_name}_tags_label_id_idx"
        ON "${schema_name}".tags (whatsapp_label_id)
      `).execute(db)
    }

    // Create whatsapp_labels table if not exists
    const tableExists = await sql<{ exists: boolean }>`
      SELECT EXISTS (
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = ${schema_name}
        AND table_name = 'whatsapp_labels'
      ) as exists
    `.execute(db)

    if (!tableExists.rows[0]?.exists) {
      await sql.raw(`
        CREATE TABLE IF NOT EXISTS "${schema_name}".whatsapp_labels (
          id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
          label_id VARCHAR(100) NOT NULL UNIQUE,
          name VARCHAR(100) NOT NULL,
          color VARCHAR(20),
      ...
      await sql.raw(`
        CREATE INDEX IF NOT EXISTS "${schema_name}_wa_labels_id_idx"
        ON "${schema_name}".whatsapp_labels (label_id)
      `).execute(db)

      await sql.raw(`
        CREATE INDEX IF NOT EXISTS "${schema_name}_wa_labels_tag_idx"
        ON "${schema_name}".whatsapp_labels (synced_tag_id)
      `).execute(db)
    }
  }
}
...
export async function down(db: Kysely<unknown>): Promise<void> {
  ...
  // Remove the whatsapp_labels table and columns from each tenant schema
  for (const { schema_name } of schemas.rows) {
    await sql.raw(`
      DROP TABLE IF EXISTS "${schema_name}".whatsapp_labels
    `).execute(db)

    await sql.raw(`
      DROP INDEX IF EXISTS "${schema_name}_wa_labels_id_idx"
    `).execute(db)

    await sql.raw(`
      DROP INDEX IF EXISTS "${schema_name}_wa_labels_tag_idx"
    `).execute(db)

    await sql.raw(`
      DROP INDEX IF EXISTS "${schema_name}_tags_label_id_idx"
    `).execute(db)

    // Remove columns from tags table
    await sql.raw(`
      ALTER TABLE "${schema_name}".tags
      DROP COLUMN IF EXISTS whatsapp_label_id,
